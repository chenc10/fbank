#Version: 1.1
#	Those functions that have some isolated nodes are also accepted and abstracted
#
...
#
    if len(EnterNode) > 1:
	print "New virtual enterNode is created"
	for node in EnterNode:
	    print node.Addr
	VEnterNode = Node(EnterNode[0].Addr + 1)
	VEnterNode.CList = EnterNode
	for node in EnterNode:
	    node.FList = [VEnterNode]
	ALLNodeList[VEnterNode.Addr] = VEnterNode
	FirstNode = VEnterNode	
    if len(ReturnNode) > 1:
